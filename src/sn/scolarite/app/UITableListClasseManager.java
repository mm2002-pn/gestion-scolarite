...
public class UITableListClasseManager extends JFrame {
	...
	public void initComponent() {
		...
		JButton listerEleveBtn = new JButton("Liste Eleves");
		listerEleveBtn.setPreferredSize(new Dimension(150, 30));
		listerEleveBtn.setBackground(new Color(46, 46, 46));
		listerEleveBtn.setForeground(new Color(255, 255, 255));
		listerEleveBtn.addActionListener(new ActionListener() {
		...
		listerEleveBtn.setFont(new Font("Times New Roman", Font.PLAIN, 20));
		panel_23.add(listerEleveBtn);
		...
		table.setModel(classeModel);
		table.setPreferredSize(new Dimension(80, 200));
		scrollPane.setViewportView(table);
		Utilitaires.center(this, getSize());
		
		
	}
	protected void onListeEleveClicked() {
		int row = table.getSelectedRow();
		if(row == -1) {
			JOptionPane.showInternalMessageDialog(null, "veuiller selectionnez une classe");
		}
		else {
			this.dispose();
			String classeName = String.valueOf(classeModel.getValueAt(row, 1));
			UIListeEleveMananger listEleve = new UIListeEleveMananger(classeName);
			listEleve.showMe();
		}
	}
